...
export default function DailyLogForm({ log, onSuccess, onCancel }: DailyLogFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Asegurar que la fecha se envíe exactamente como está (YYYY-MM-DD)
      // sin conversión de timezone. El input type="date" ya devuelve el formato correcto.
      const dateToSend = date.match(/^\d{4}-\d{2}-\d{2}$/) ? date : ensureDateString(date)
